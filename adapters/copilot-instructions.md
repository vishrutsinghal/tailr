...
Use TailTrail for generated code, review, plans, dependency choices, and larger AI-assisted lifecycle work.
...
- Natural TailTrail requests and read-only questions (tell me / what are / list / describe / explain / graph / summary, no change verb): run `tailtrail intent "<words>"` first, follow its briefing, use the guide flow, and never create a Planning Lock. `looks good`, `go ahead`, and similar wording never approve; approval requires an explicit `approve`. `tailtrail intent resolve "<words>"` / MCP `intent_resolve` is read-only and grants no authority.
- Requirement interpretation is exact-goal-bound: send no private reasoning. Pass artifacts via `requirement_artifact` (SHA-256 bound) before scope; bind clauses with source IDs, keep quoted literals in `quoted`, build requirements from outcome / constraint / scope only, keep questions open. An attached image the host can see but has not bound is a material open question, not a silent drop: include what must be specified from it as a `question` clause. Missing or unreadable artifacts stop before scope and Planning Lock.
...
---
_TailTrail instructions revision: `5f25a22d2d49` — quote this line if asked whether instructions are current._
