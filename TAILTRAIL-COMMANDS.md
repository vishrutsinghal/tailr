...
`--profile` and `--surface` are independent on `install-local.py`: `--profile` selects the installation context or assistant host (`copilot`, `aidlc`, `hooks`, `full`), while `--surface` selects file breadth (`core` or `extended`). Extended is the default and matches the full pack behavior; Core is the smaller first-run pack.
