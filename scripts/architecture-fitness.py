...
def assess(root:Path,run_id:str,changed:list[str],profile_path:Path|None=None)->dict[str,Any]:
 ...
 findings=[]; rules=profile.get("rules",[]) if isinstance(profile,dict) else []
 ...
 payload={"schema_version":"1","type":"tailtrail-architecture-fitness","run_id":run_id,"changed_paths":paths,"profile":profile_path.as_posix() if profile_path else None,"findings":findings,"complete":not findings,"evidence_label":"local-ast + approved-contract","boundary":"deterministic path/import rules only; source and focused tests remain final proof"}
