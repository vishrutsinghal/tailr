...
L=ledger()
TIERS={"unit","component","integration","contract","e2e","infrastructure","release-smoke"};OUTCOMES={"pass","fail","blocked","timed-out","unavailable"}
def main()->int:
 p=argparse.ArgumentParser();p.add_argument("--root",type=Path,default=Path.cwd());p.add_argument("--run-id");p.add_argument("--requirement-uid",required=True);p.add_argument("--tier",choices=sorted(TIERS),required=True);p.add_argument("--command",required=True);p.add_argument("--outcome",choices=sorted(OUTCOMES),required=True);p.add_argument("--environment",required=True);p.add_argument("--asserted-behavior",required=True);p.add_argument("--artifact",default="");p.add_argument("--evidence-label",default="local-command");p.add_argument("--output",type=Path);a=p.parse_args()
 payload={"schema_version":"1","type":"tailtrail-validation-evidence-receipt","requirement_uid":a.requirement_uid,"tier":a.tier,"command":a.command,"outcome":a.outcome,"environment":a.environment,"asserted_behavior":a.asserted_behavior,"artifact_path":a.artifact,"evidence_label":a.evidence_label,"created_at":datetime.now(timezone.utc).replace(microsecond=0).isoformat()}
 if a.run_id:
  directory=L.state_dir(a.root.resolve(),a.run_id)/"validation-receipts";directory.mkdir(parents=True,exist_ok=True);path=directory/f"{a.requirement_uid}-{a.tier}-{len(list(directory.glob('*.json')))+1}.json";L.atomic_json(path,payload);L.append_event(a.root.resolve(),a.run_id,"validation_receipt",{"artifact":path.relative_to(a.root.resolve()).as_posix(),"requirement_uid":a.requirement_uid,"tier":a.tier,"outcome":a.outcome,"environment":a.environment,"command":a.command});payload["run_artifact"]=path.as_posix()
