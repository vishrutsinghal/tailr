...
def evidence_tiers():
 s=importlib.util.spec_from_file_location("ci_evidence_tiers",ROOT/"scripts"/"evidence-tiers.py");m=importlib.util.module_from_spec(s);assert s and s.loader;s.loader.exec_module(m);return m
EVIDENCE_TIERS=evidence_tiers();TIERS=set(EVIDENCE_TIERS.CANONICAL_EVIDENCE_TIERS);OUTCOMES={"pass","fail","blocked","timed-out","unavailable"}
def ledger():
...
def ingest(root:Path,run_id:str,input_path:Path)->dict[str,Any]:
 ...
 for item in items:
  ...
  tier=EVIDENCE_TIERS.normalize(item["tier"])
  if item["requirement_uid"] not in known or tier not in TIERS or item["outcome"] not in OUTCOMES:raise ValueError("CI result has unknown requirement, tier, or outcome")
  item={**item,"tier":tier}
