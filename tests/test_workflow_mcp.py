...
class WorkflowMcpTests(unittest.TestCase):
 ...
 def test_create_finishes_only_the_approved_saved_start_draft_and_preflights_it(self):
  with tempfile.TemporaryDirectory() as temp:
   ...
   owner=root/"src"/"mcp_adapter.py"; owner.parent.mkdir(parents=True,exist_ok=True); owner.write_text("def adapt(value):\n return value\n",encoding="utf-8")
   command=[sys.executable,(ROOT/"scripts"/"task-start.py").as_posix(),"implement a bounded MCP adapter","--root",root.as_posix(),"--changed","src/mcp_adapter.py","--planning-run-id",run_id,"--format","json"]
 ...
 def test_closure_control_runs_canonical_finalizer_before_linking_receipt(self):
  with tempfile.TemporaryDirectory() as temp:
   ...
   closure_input=root/"closure-input.json"; closure_input.write_text(json.dumps({"schema_version":"1","type":"tailtrail-execution-closure-input","run_id":"closure-run","changed_paths":["src/service.py","tests/test_service.py"],"receipts":[{"requirement_uids":[uid],"tier":"unit","command_label":"safe proof","command":"python -m unittest tests.test_service","outcome":"pass","environment":"local","asserted_behavior":"Safe behavior.","evidence_label":"ci-receipt","evidence_quality":"attested","artifact":"proof.json"}]}),encoding="utf-8"); recorder.record(root,closure_input)
