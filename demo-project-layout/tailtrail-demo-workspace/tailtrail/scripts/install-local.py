...
PROFILES = ("inspect", "generic", "copilot", "aidlc", "hooks", "full")
DEFERRED = [
    ...
    "Assistant-specific installers beyond the existing Copilot managed pack are not implemented.",
...
def copilot_steps(target: Path, pack_dir: str, force: bool) -> list[Step]:
    command = [
        sys.executable,
...
def steps_for(profile: str, target: Path, pack_dir: str, depth: str, team_mode: str, force: bool) -> list[Step]:
    if profile == "inspect":
        return []
    if profile == "generic":
        return generic_steps(target, force)
    if profile == "copilot":
        return copilot_steps(target, pack_dir, force)
    if profile == "aidlc":
    ...
    if profile == "full":
        return [
            *copilot_steps(target, pack_dir, force),
...
def print_plan(profile: str, target: Path, steps: list[Step], dry_run: bool) -> None:
    print("TailTrail local installer")
...
def main() -> int:
    ...
    steps = steps_for(profile, target, args.pack_dir, args.depth, args.team_mode, args.force)
    print_plan(profile, target, steps, args.dry_run)
    sys.stdout.flush()
