...
def workspace_overlays(root: Path, target_files: list[Path], scanner_files: list[Path]) -> list[dict[str, Any]]:
    overlays: list[dict[str, Any]] = []
    changed = set(git_changed(root))
    for path in target_files:
        rel = safe_relative(path, root)
        if rel and rel in changed:
            overlays.append({"type": "git-changed-file", "path": rel, "confidence": "git-diff"})
...
def cache_path(root: Path, override: Path | None) -> Path:
    ...
    return root / ".tailtrail" / "code-graph-cache.json"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for name in ("map", "status", "refresh"):
        ...
        sub.add_argument("--cache", type=Path, default=None, help="Override cache path. Defaults to .tailtrail/code-graph-cache.json.")
