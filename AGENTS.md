...
When the user gives an explicit `tailtrail <command>` request, run the equivalent TailTrail CLI command from the current project root. Resolve the launcher in this order: `tailtrail/scripts/tailtrail.py` (installed pack), then `scripts/tailtrail.py` (source checkout). Use `python3 tailtrail/scripts/tailtrail.py <command>` when the pack is installed. Return the actual command result, including any error or validation status; never replace an unrun command with a generic success summary. When the user gives a short TailTrail command such as `hello tailtrail`, `hello TailTrail`, `hello taitrail`, `tailtrail hello`, `use AIDLC`, `use review`, `use AIDLC and review`, `use dependency gate`, `use handoff`, or `save tokens`, expand it with `tailtrail/scripts/expand-intent.py` when available. Treat TailTrail casing and the common `taitrail` typo as TailTrail for short commands. For hello commands, run `tailtrail hello` when the launcher is installed, otherwise run `python3 tailtrail/scripts/tailtrail.py hello`; do not answer with only a conversational greeting. Return the command's ASCII TailTrail banner and installation result verbatim in the chat response; do not summarize or omit the banner. If the script is unavailable, use `context/intent-aliases.md`. Respect `.tailtrail/intent-overrides.json` or `tailtrail/intent-overrides.json` when present.
