...
The Codex guidance installer writes `AGENTS.md` only. It does not write global Codex configuration, install a plugin, change IDE settings, or enable hooks. The plugin installer writes `.codex-plugin/plugin.json` and the `skills/` source needed for native `@tailtrail` and `@tailtrail-review` invocation. If you only want portable project guidance, you can also copy `AGENTS.md` into the root of the target project.
